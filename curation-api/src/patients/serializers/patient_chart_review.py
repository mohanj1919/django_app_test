...
class PatientChartReviewListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('cohort_id', 'cohort', 'project', 'curator', 'curation_status', 'patient_id')
...
class PatientChartReviewQuestionCreateSerializer(serializers.Serializer):
    ...
    annotation_text = serializers.CharField(
        required=False, allow_null=True, allow_blank=True)
    annotation_responses = serializers.CharField(
        required=False, allow_null=True, allow_blank=True)
    class Meta:
        fields = ('crf_template_question_id', 'responses', 'annotation_text', 'annotation_responses')
    ...
    def _create_chart_review_question_response(self, validated_data, chart_review):
        ...
        try:
            patient_chart_review_question_data = {
                ...
                'annotation_text': validated_data.get('annotation_text'),
                'annotation_responses': validated_data.get('annotation_responses')
            }
...
class CRFTemplateGetSerializer(serializers.ModelSerializer):
    class Meta:
        model = CRFTemplate
        fields = ('id', 'name', 'description')

class ProjectChartReviewDataSerializer(serializers.ModelSerializer):
    cohort = CohortListCreateSerializer()
    crf_template = CRFTemplateGetSerializer()
    # curator = UserRetrieveSerializer()
    project = ProjectSeralizer()
    class Meta:
        model = PatientChartReview
        fields = ( 'project', 'cohort', 'crf_template', 'status')
