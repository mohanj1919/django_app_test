...
class ProjectCuratorSerializer(serializers.ModelSerializer):
    ...
    def _validate_project_curator_details(project_id, curators):
        ...
        if project_patients.count():
            _curators = list(project_patients.values_list('curator__first_name', 'curator__last_name'))
            _curators = ', '.join(map(' '.join, _curators))
            message = "Unable to remove curator. Curator account '{}' has incomplete patient records.".format(_curators)
            res = {'errors': {}}
