...
from ..serializers.patient_chart_review import ProjectChartReviewDataSerializer
...
class PatientChartReviewView(GenericViewSet,
    ...
    def get_serializer_class(self):
        ...
        if self.action == 'get_completed_project_chart_reviews':
            return ProjectChartReviewDataSerializer
    ...
    def get_chart_review_data(self, chart_review):
        response_data = {}
        chart_review_data = chart_review
        response_data['project'] = ProjectDataSeralizer(
            chart_review_data.project).data
        response_data['cohort'] = CohortDataSerializer(
            chart_review_data.cohort).data
        response_data['patient'] = PatientDataSerializer(
            chart_review_data.patient).data
        response_data['curator'] = UserDataSerializer(
            chart_review_data.curator).data
        response_data['status'] = chart_review_data.status
        crf_templates = []
        crf_templates.append(self.get_chart_review_crf_template_data(chart_review))
    ...
    def get_chart_review_response(self, request):
        ...
        patient_id = reqParams.get('patient_id', None)
        crfTemplate_id = reqParams.get('crf_template_id', None)
        ...
        try:
            ...
            completed_chart_reviews = []
            for chart_review in queryset:
                completed_chart_reviews.append(self.get_chart_review_data(chart_review))

            return Response(completed_chart_reviews, status=status.HTTP_200_OK)
        except PatientChartReview.DoesNotExist:
            return Response({}, status=status.HTTP_200_OK)
    ...
    def search_chart_review_data(self, project, cohort, patient, status, user):
        """
        ...
        if GenericViewSet._is_curator(user):
            # get the curator's project details from queryset if the request user is curator.
            queryset = queryset.filter(curator_id=user.id)
    ...
    def get_completed_chart_reviews(self, request):
        ...
        status = req_params.get(
            'status', default=ProjectCohortPatient.CurationStatus.completed)

        queryset = self.search_chart_review_data(
            project, cohort, patient_id, status, request.user)
    ...
    def sort_chart_reivews(self, queryset, sort_name, sort_order):
        ...
        """
        if sort_name == 'cohort_name':
            sort_name = 'cohort__name'
        if sort_name == 'project_name':
            sort_name = 'project__name'
        if sort_name == 'curator_email':
    ...
    def search_project_chart_review_data(self, project, cohort, crf_template, user, search_param):
        """
        Searches completed project chart review data
        """
        queryset = ProjectCohortPatient.objects.filter(
            curation_status=ProjectCohortPatient.CurationStatus.completed
        ).order_by('patient_id', '-updated_on')
        ...
        cohort_ids = queryset.values_list('cohort_id', flat=True)
        project_ids = queryset.values_list('project_id', flat=True)

        compelted_crf_templates = PatientChartReview.objects.filter(
            status=PatientChartReview.StatusType.completed,            
            is_active=True,
            cohort_id__in=cohort_ids,
            project_id__in=project_ids).distinct('crf_template')

        if crf_template is not None:
            compelted_crf_templates = compelted_crf_templates.filter(
                Q(crf_template__name__icontains=crf_template))

        if search_param is not None:
            compelted_crf_templates = compelted_crf_templates.filter(Q(cohort__name__icontains=search_param) | Q(
                project__name__icontains=search_param) | Q(
                    crf_template__name__icontains=search_param
                ))

        # queryset = queryset.distinct('patient')
        return compelted_crf_templates

    def sort_project_chart_reivews(self, queryset, sort_name, sort_order):
        """
        Sorts the chart review data by project name, crf template name
        """
        if sort_name == 'project':
            sort_name = 'project__name'
        if sort_name == 'cohort':
            sort_name = 'cohort__name'
        if sort_name == 'crf_template':
            sort_name = 'crf_template__name'
    ...
    @list_route(methods=['get'])
    def get_completed_project_chart_reviews(self, request):
        """
        Gets all completed patient chart review per project CRF template based on serach criteria.
        """
        req_params = request.GET
        project = req_params.get('project', None)
        cohort = req_params.get('cohort', None)
        crf_template = req_params.get('crf_template', None)
        search_param = req_params.get('searchParam', default=None)
        sort_name = req_params.get('sortName', default=None)
        sort_order = req_params.get('sortOrder', default='asc')

        queryset = self.search_project_chart_review_data(project, cohort, crf_template, request.user, search_param)
