...
from ..models import Patient, ProjectCohortPatient, ProjectCohort
...
class CohortPatientsListView(CustomGenericAPIView, ListModelGenericViewMixin):
    ...
    def create_project_patient(self, patient):
        project_cohorts = ProjectCohort.objects.filter(cohort_id=patient.cohort_id).distinct('project_id')
        prepared_project_patients = []
        for project_cohort in project_cohorts:
            prepared_project_patient = ProjectCohortPatient(
                project_id=project_cohort.project_id,
                cohort_id=project_cohort.cohort_id,
