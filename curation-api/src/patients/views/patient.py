...
class PatientView(ListModelViewMixin,
    ...
    def fetch_next_patient(self, request):
        ...
        if assigned_patient is None:
            ...
            new_patient = ProjectCohortPatient.objects.get_patient_from_pending_list(project_id, cohort_id)
            if new_patient is not None:
                ...
                new_patient.save()
                logger.info('Updating the curation status for the patient: %s ', new_patient.patient_id)
    ...
    def get_assigned_patient_details(self, curator_id, project_id, cohort_id):
        ...
        try:
            logger.info(
                'Getting already assigned patient assinged to curator %d, project %s, cohort %s',
                curator_id,
                project_id,
                cohort_id)
    ...
    def unassign_patient(self, request):
        """
        unassings patient assigned to curator
        """
        try:
            data = request.data
            patient_id = data.get('patient_id', None)
            curator_id = data.get('curator_id', None)
            project_id = data.get('project_id', None)
