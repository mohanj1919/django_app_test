...
class UserRetrieveSerializer(serializers.ModelSerializer):
    class Meta:
        model = CurationUser
        fields = ('id', 'email', 'first_name', 'last_name', 'mfa_type', 'phone_number',
...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'email', 'first_name', 'last_name', 'mfa_type',
                  ...
                  'curator_chart_reviews', 'require_mfa', 'is_account_locked')
...
class CreateUserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'email', 'first_name', 'last_name',
                  'phone_number', 'require_mfa', 'mfa_type', 'groups', 'is_active', 'is_account_locked')
    ...
    def create(self, validated_data):
        ...
        data['phone_number'] = None if phone_number is None or phone_number == "" else phone_number
        mfa_type = data.get('mfa_type', None)
        data['require_mfa'] = False if mfa_type is None or mfa_type == 'none' else True
        data['password_expiry_on'] = datetime.now(timezone.utc) + timedelta(days=30)
    ...
    def update(self, instance, validated_data):
        ...
        validated_data['phone_number'] = None if phone_number is None or phone_number == "" else phone_number
        instance.phone_number = validated_data.get('phone_number')
        mfa_type = data.get('mfa_type', None)
        instance.require_mfa = False if mfa_type is None or mfa_type == 'none' else True
        instance.save()
        return instance
    ...
    def update(self, instance, validated_data):
        ...
        if password is not None:
            ...
            if can_update_user_password(instance, new_password):
                ...
                error_response['errors']['new_password'] = [
...
def can_update_user_password(user, new_password):
    ...
    last_passowrds = PasswordHistory.objects.filter(
        user_id=user.id).order_by('-id')[:password_histpry_check]
