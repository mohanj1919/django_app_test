...
import { Button, ControlLabel, FormGroup, FormControl, InputGroup, Modal, HelpBlock } from 'react-bootstrap'
...
class ManageCaseReportForm extends Component {
  ...
  render () {
    ...
    var ParentResponseList = (cell, row) => {
      return (
        <FormControl name='table_parent_condition' componentClass='select' disabled={!(row && row.parent_question)}
      ...
      return (
        <div>
          <Button onClick={() => this.props.AddModal()} bsStyle='primary'>
            <i className='fa fa-plus' aria-hidden='true' /> Add Question
                    </Button>
          {props.exportCSVBtn}
        </div>
      )
    ...
    var editableCol = (cell, row) => {
      if (row && row.parent_question) { return true }
      return false
    }
    var responsesEditCol = (cell, row) => {
      if (row && row.question_type && row.question_type !== 'text') { return true }
      return false
    }
    var options = {
                  ...
                  {this.props.isFilesVisible
                                        ? <div className='file-name'>
                                          {this.props.uploadedfiles ? this.props.uploadedfiles.map((fd, i) =>
                                            <span title={fd.name} key={i}>{fd.name} <i className='fa fa-times' onClick={() => this.props.RemoveFile(fd.name)} /><br /></span>
                                            ) : null}
                                        </div>
                                        : null
                                    }
          ...
          <div className='col-lg-offset-1 col-lg-10 col-md-12'>
            <BootstrapTable
              ...
              csvFileName={this.props.templateDetails && this.props.templateDetails.template_name ? this.props.templateDetails.template_name + '.csv' : 'questions_data' + '.csv'}
                            >
              <TableHeaderColumn dataField='question_id' width='6%' isKey dataSort>Id</TableHeaderColumn>
              <TableHeaderColumn dataFormat={tableCell} width='23%' dataField='text'dataSort>Question Text</TableHeaderColumn>
              ...
              <TableHeaderColumn dataFormat={tableCell} width='10%' dataField='responses' editable={responsesEditCol} dataSort>Responses</TableHeaderColumn>
              ...
              <TableHeaderColumn dataFormat={ParentResponseList} width='10%' dataField='parent_condition' editable={false} dataSort>Parent Condition</TableHeaderColumn>
              <TableHeaderColumn dataFormat={tableCell} width='10%' dataField='parent_response' editable={editableCol} dataSort>Parent Response</TableHeaderColumn>
              <TableHeaderColumn dataFormat={tableCell} width='10%' editable={{ type: 'textarea', validator: NoteValidator }} dataField='note' dataSort>Note</TableHeaderColumn>
            ...
            <div className='submit-crf-style'>
              <Button bsStyle='success'
                disabled={!(this.props.templateDetails &&
                                    ...
                                    this.props.templateDetails.questionData.length > 0)}
                                    ...
                                    ? <FormGroup
                                      ...
                                      {this.props.newQuestion && this.props.newQuestion.responses
                                            ? this.props.newQuestion.responses.map((nq, i) => (
                                              <span key={i}><label>{nq}</label> <i className='fa fa-times' onClick={() => this.props.RemoveResponse(i, 'responses')} /><br /></span>
                                            ))
                                            : null}
                                    </FormGroup> : null
                                }
                                      ...
                                      <FormGroup
                                        ...
                                        {this.props.newQuestion && this.props.newQuestion.parent_response
                                                ? this.props.newQuestion.parent_response.map((nq, i) => (
                                                  <span key={i}><label>{nq}</label> <i className='fa fa-times' onClick={() => this.props.RemoveResponse(i, 'parent_response')} /><br /></span>
                                                ))
                                                : null}
            ...
            <Modal.Footer>
              <Button bsStyle='success' onClick={() => this.props.AddQuestion()}
                disabled={!(this.props.newQuestion &&
                                    this.props.newQuestion.text)}>Add</Button>
            </Modal.Footer>
          </Modal>
