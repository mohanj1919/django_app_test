...
export function GetCRFDetails (crfId) {
  return (dispatch, getState) => {
    ...
    if (getState().managecasereportform.crfId) {
      let crfgeturl = `/clinical/crftemplates/${getState().managecasereportform.crfId}/`
      dispatch({
        type: TOGGLE_LOADING,
        payload: true
      })
...
export function SaveCRFTemplate () {
  return (dispatch, getState) => {
    ...
    let addUrl = getState().managecasereportform.crfId ? `/clinical/crftemplates/${getState().managecasereportform.crfId}/` : '/clinical/crftemplates/'
      ...
      .then(function (response) {
        ...
        if (!_.isEmpty(response.data.errors)) {
          dispatch({
            ...
            confirmText: 'Clone',
            showType: 'warning',
            showMessage: `${response.data.errors} Clone to create new template`,
...
export function RemoveFile (filename) {
  ...
  filedetails.map((fd, i) => {
    if (filename != fd.createdtype) {
      ...
      noOfRows++
      delParentIds.push(parseInt(fd.parent_question))
    }
  })
...
const ACTION_HANDLERS = {
  ...
  [FILE_SELECTED]: (state, action) => {
    if (action.payload) {
      ...
      if ([...new Set(files.map(item => item.name.toLowerCase()))].indexOf(action.payload.name.toLowerCase()) == -1) { files.push(action.payload) }
      ...
      if (state.templateDetails && state.templateDetails.questionData) {
        state.templateDetails.questionData.map((qd, i) => {
          if (state.crfId) {
            ...
            if (qd.createdtype == 'custom') { quesData.push(qd) } else {
  ...
  [REMOVE_ADD_FILE]: (state, action) => {
    ...
    delParentIds = _.map(deleteableQues, function (q) {
      return q.question_id
    })
    if (state.templateDetails && state.templateDetails.questionData) {
      let quesId = 1
      ...
      files.map((f) => {
        if (f.name) {
          if (filesUploaded.indexOf(f.name.toLowerCase()) == -1) {
            uploadble = !!(files && files.length > 0)
          }
        }
      })
    }
