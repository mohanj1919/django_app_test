...
import { Panel, DropdownButton, MenuItem, Button, FormControl } from 'react-bootstrap'
            ...
            <div className='row row-eq-height cohort-name-heading'>
              <div className='col-sm-12'>
                <label className='cohort-name'><span className='text-info'>{_.upperFirst(this.props.projectName) }</span>
                  {this.props.patientData ? <span><i className='fa fa-chevron-right' /> Patient ID: {this.props.patientData.patient_id} </span> : null}
                </label>
              </div>
            </div>
              ...
              <div className='col-md-4 crf-div'>
                {!_.isEmpty(this.props.availableCRFNames)
                  ? <div className='template-selection'>
                    <DropdownButton id='crfSection' title={this.props.availableCRFS.name?this.props.availableCRFS.name:''} className={this.props.isCompletedRequired || _.isEmpty(this.props.availableCRFS) ? 'disable-form' : null}>
                    ...
                    <div className={'questions-content'}>
                      <div className={this.props.isReadOnly ? 'disable-form' : null}>
                        <CRFQuestion
                          ...
                          encountersdates={encountersdates} />
                      </div>
                    </div>
                    {!this.props.isReadOnly
                      ? <div className='crf-controls'>
                        {this.props.pendingCRFS && this.props.pendingCRFS > 1
                          ? <span>
                            <Button bsStyle='default' onClick={this.props.SubmitTemplate}>Next CRF <i className='fa fa-chevron-right' /></Button>
                          </span> : null}
                        ...
                        <div className='row'>
                          {this.props.patientData ? Object.keys(this.props.patientData).map((pd, index) => (
                            typeof (this.props.patientData[pd]) !== 'object' ? (
                              pd.toLowerCase()!='id'?
                              <ShowDetails text={pd} value={this.props.patientData[pd] ? this.props.patientData[pd] : 'N/A'} key={'pd' + index}/>:null
                            ) : null
                          )) : null}
                        </div>
                      </Panel>
