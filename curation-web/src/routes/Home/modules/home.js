...
export function PasswordExpirationCheck () {
  return (dispatch) => {
    let passwordExpiryOn = localStorage.getItem('password_expiry_on')
    if (passwordExpiryOn) {
      let passwordExpiraryDate = new Date(passwordExpiryOn)
      let today = new Date()
...
export function PageChanged (page, sizePerPage) {
  return (dispatch) => {
    dispatch({ type: SET_PROPS, payload: { key: 'page', value: page || 1 } })
    dispatch({ type: SET_PROPS, payload: { key: 'sizePerPage', value: sizePerPage } })
