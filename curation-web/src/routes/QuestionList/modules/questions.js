...
import { TOGGLE_NOTIFICATION, TOGGLE_LOADING } from '../../../modules/global.js'
...
export function LoadQuestionsData (page, sizePerPage) {
  return (dispatch, getState) => {
    ...
    if (page && sizePerPage) {
      ...
      dispatch({ type: SET_PROPS, payload: { key: 'pageNumber', value: page } })
...
export function PageChanged (page, sizePerPage) {
  return (dispatch) => {
    ...
    dispatch({ type: SET_PROPS, payload: { key: 'pageNumber', value: page || 1 } })
    dispatch({ type: SET_PROPS, payload: { key: 'sizePerPage', value: sizePerPage } })
    dispatch(LoadQuestionsData())
  }
}
export function SizePerPageListChange (sizePerPage) {
  return (dispatch) => {
    dispatch({ type: SET_PROPS, payload: { key: 'pageNumber', value: 1 } })
    dispatch({ type: SET_PROPS, payload: { key: 'sizePerPage', value: sizePerPage } })
    dispatch(LoadQuestionsData())
  }
}
...
const ACTION_HANDLERS = {
  [USER_INPUT_CHANGE]: (state, action) => {
    ...
    if (action.payload.key != 'responses') {
      ...
      return Object.assign({}, state, { [objType]: newQues, responses: '', showOptions: !!(newQues.type && newQues.type.value && newQues.type.value != '0' && newQues.type.value != 'text' && newQues.type.value != 'date') })
    } else {
      ...
      return Object.assign({}, state, {
        ...
        showOptions: !!(state[objType].type && state[objType].type.value != '0' && state[objType].type.value != 'text' && state[objType].type.value != 'date')
      })
    }
  },
  ...
  [TOGGLE_MODAL]: (state, action) => {
    if (action.payload) {
      ...
      return Object.assign({}, state, {
        ...
        selectedObject: Obj,
        showOptions: !!(Obj.type && Obj.type.value != '0' && Obj.type.value != 'text' && Obj.type.value != 'date'),
        isDeletable: action.key == 'delete'
      })
    }
    ...
    if (validatingObj) {
      ...
      if (validatingObj.type && validatingObj.type.value && (!(validatingObj.type.value == 'text' || validatingObj.type.value == 'date' ||
        ...
        if (!validatingObj.responses || !validatingObj.responses.value || validatingObj.responses.value.length == 0) {
          ...
          validatingObj.responses.help = 'At least one Response is required'
          valid = false
        }
      }
  ...
  [SAVE_QUESTION]: (state, action) => {
    ...
    saveObj.is_active = !state.isDeletable
    return Object.assign({}, state, {
      questionOptions: null,
      showOptions: false,
      responseMinValue: null,
      responseMaxValue: null,
      selectedObject: null,
      showAddModal: !state.showAddModal,
      newQuestion: null,
      saveObject: saveObj
