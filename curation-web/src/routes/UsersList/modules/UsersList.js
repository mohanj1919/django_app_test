...
var _getRoleIdByRole = function (state, role) {
  var userRole = state.allRoles.find((item) => {
    ...
    return new Promise((resolve) => {
      if (getState().userslist) {
        ...
        if (!saveObj.mfa_type) {
          ...
          if (saveObj.mfa_type.toUpperCase() == config.MFA_TYPE.SMS) {
...
export function ConfirmResetUserPassword () {
  return (dispatch, getState) => {
    dispatch({ type: TOGGLE_LOADING, payload: true })
    axios.get(config.api.reset_password,
      {
        params: { email: getState().userslist.userResetEmail }
      }).then((response) => {
        dispatch({ type: TOGGLE_NOTIFICATION,
          payload: true,
          showTime: 3000,
